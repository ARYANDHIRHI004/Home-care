...
export function MonthlyRevenueTrendChart({ invoices = [], payments = [] }) {
    const { months, list } = last6Months();
    const dataMap = list.reduce((acc, m) => ({ ...acc, [m]: { month: m, revenue: 0, collections: 0 } }), {});

    invoices.forEach((inv) => {
        if (!inv.createdAt) return;
        const m = months[new Date(inv.createdAt).getMonth()];
        if (dataMap[m]) dataMap[m].revenue += (inv.total || 0);
    });

    payments.forEach((p) => {
        if (!p.createdAt || p.status !== 'verified') return;
        const m = months[new Date(p.createdAt).getMonth()];
        if (dataMap[m]) dataMap[m].collections += (p.amount || 0);
    });
...
export function RevenueVsExpensesChart({ invoices = [], expenses = [] }) {
    const { months, list } = last6Months();
    const dataMap = list.reduce((acc, m) => ({ ...acc, [m]: { month: m, revenue: 0, expenses: 0 } }), {});
    ...
    expenses.forEach((e) => {
        ...
        if (dataMap[m]) dataMap[m].expenses += (e.amount || 0);
    });
...
export function RevenueByCategoryChart({ bookings = [] }) {
...
export function RevenueByPaymentMethodChart({ payments = [] }) {
    const colors = ['#2563EB', '#10B981', '#F59E0B', '#8B5CF6'];
    const verified = payments.filter((p) => p.status === 'verified');
    const methodMap = verified.reduce((acc, p) => {
