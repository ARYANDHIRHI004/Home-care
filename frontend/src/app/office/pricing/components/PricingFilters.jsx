...
export default function PricingFilters({ searchQuery, onSearchChange }) {
    const [isFiltersOpen, setIsFiltersOpen] = useState(false);

    const filterOptions = [
        { label: 'Category', options: ['All Categories', 'Deep Cleaning', 'Plumbing', 'Electrical', 'AC Repair', 'Pest Control'] },
        { label: 'Status', options: ['All Statuses', 'Active', 'Scheduled', 'Expired'] },
        { label: 'Price Range', options: ['Any Price', 'Under ₹500', '₹500 - ₹2000', 'Over ₹2000'] },
    ];
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 mb-6 sticky top-20 z-10 min-w-0">
            ...
            {/* Row 2: Filters */}
            <div className={`${isFiltersOpen ? 'grid' : 'hidden'} lg:grid grid-cols-2 sm:grid-cols-3 gap-2 mt-3`}>
