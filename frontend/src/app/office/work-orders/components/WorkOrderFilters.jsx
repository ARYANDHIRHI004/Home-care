...
export default function WorkOrderFilters({ searchQuery, onSearchChange }) {
    const [isFiltersOpen, setIsFiltersOpen] = useState(false);

    const filterOptions = [
        { label: 'Status', options: ['All Statuses', 'Pending Assignment', 'Assigned', 'Accepted', 'On The Way', 'Arrived', 'Work Started', 'Paused', 'Completed', 'Closed', 'Cancelled'] },
        { label: 'Priority', options: ['All Priorities', 'Low', 'Medium', 'High', 'Critical'] },
        { label: 'Category', options: ['All Categories', 'Cleaning', 'Plumbing', 'Electrical', 'AC Repair', 'Appliance', 'Pest Control'] },
        { label: 'Date', options: ['Any Time', 'Today', 'Tomorrow', 'This Week', 'Next Week'] },
        { label: 'Payment', options: ['All Payments', 'Pending', 'Partial', 'Paid'] },
        { label: 'Invoice', options: ['All Invoices', 'Un-invoiced', 'Draft', 'Sent', 'Outstanding'] },
    ];
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 mb-6 sticky top-20 z-10 min-w-0">
            ...
            <div className="flex gap-3 min-w-0">
                <div className="relative flex-1 min-w-0">
                    ...
                    <input
                        ...
                        placeholder="Search Work Order ID, Booking ID, Customer, Phone..."
                    />
                </div>
                <button
            ...
            {/* Row 2: Filters */}
            <div className={`${isFiltersOpen ? 'grid' : 'hidden'} lg:grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-2 mt-3`}>
